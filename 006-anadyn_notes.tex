%<*Interet>
\textbullet~~Les informations générées dynamiquement ne peuvent être récupérée par 
 une analyse statique\\
\textbullet~~Certaines méthodes d'obfuscation sont difficiles à déchiffrer. 
  Cependant, étant donné qu'au sein de l'application, ces valeurs vont être 
   déchiffrées, on peut essayer de la récupérer à ce moment\\
\textbullet~~ Certaines requêtes ne peuvent être déciffrée par un MITM,
 on peut alors essayer de lire les données de la requête au moment de l'envoie  
 ou de la réception de la requête
%</Interet>

%<*Utilisation>
\bigbreak
\textbf{\uline{Débugger:}}Permet de faire mettre des breakpoints
Cependant, étant donné qu'on a pas accès au code source,
 ...
 \textbf{\uline{Mémoire:}} Permet de récupérer certaines valeurs 
%</Utilisation>

%<*Environnement>
\bigbreak
\textbf{\uline{Emulateur:}} plus de facilité pour le rooter ainsi qu'installer Xposed\\
\textbf{\uline{Root, Xposed:}}\\
\textbf{\uline{Inspeckage:}}Démarrer des activités non déclarées, Désactiver le SSL, remplacer des paramètres d'application...
\textbf{\uline{Android Device Monitor:}} Outil intégré à Android Studio 
